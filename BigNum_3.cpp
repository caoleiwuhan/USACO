...
struct BigNumber
{
	...
	int Data[Capacity];
	BigNumber() : Len(0) {}
	BigNumber(const BigNumber& V) : Len(V.Len) {
		memcpy(Data, V.Data, Len * sizeof *Data);
	}
	BigNumber(int V) : Len(0) {
...
istream& operator>>(istream& In, BigNumber& V)
{
	char Ch;
	for (V = 0; In >> Ch;)
	{
...
ostream& operator<<(ostream& Out, const BigNumber& V)
{
	int I;
...
//带负数
int get_val_negative_int(int & ret) {
	ret = 0;
	char ch;
	bool neg = false;
...
{	
#ifdef _MY_DEBUG_
	string inputStr = "C:/Users/cao/Desktop/算法/USACO/1.txt";
